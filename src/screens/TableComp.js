      ...
      if (parsedData.success && parsedData.attendanceRecords) {
        ...
        const mappedData = userData.map((item,index)=> {
          ...
          return {
            ...
            Name:<TouchableOpacity style={{flexDirection:"row"}} onPress={()=>handleRowPress(index,item.employeeId.name)}>
             
              <Text> {item.employeeId.name} </Text>
             {len>1?<TableIcon color="black" name={expandedRows.includes(index)?'chevron-small-up':'chevron-small-down'} size={20} />:null}
